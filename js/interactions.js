...
(function () {
  ...
  function initContactForm() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      [['nome', nome], ['servico', servico], ['bairro', bairro], ['descricao', descricao]].forEach(function (pair) {
        setError(pair[1], errors[pair[0]] || '');
      });

      var firstKey = ['nome', 'servico', 'bairro', 'descricao'].filter(function (k) { return errors[k]; })[0];
      if (firstKey) {
        if (status) status.classList.remove('is-visible');
        form.querySelector('#os-' + firstKey).focus();
        return;
      }
